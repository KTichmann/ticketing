			...
			.then(result => {
				...
				if (row === undefined) {
					...
					const verifyUserQuery = {
						text: "UPDATE users SET verified=TRUE where username=$1",
						values: [username]
					};
					client
						...
						.then(result => {
							//TODO: Change to html response from another folder
							res.send(
								"Email address verified successfully, please visit this link to log in <a href='http://www.google.com'>Log In</a>"
							);
						})
						.catch(error =>
							//TODO: Change to html response from another folder
							res.json({ success: false, message: "error validating user: 1" })
						);
				}
			})
			.catch(error => {
				//TODO: Change to html response from another folder
				res.json({ success: false, message: "error validating user: 2" });
			});
	}
